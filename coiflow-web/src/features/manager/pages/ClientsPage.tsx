    ...
    <div className="space-y-6">
      ...
      <Modal isOpen={isCreateOpen} onClose={() => setIsCreateOpen(false)} title="Nouveau client">
        <form onSubmit={handleCreate} className="space-y-4">
          ...
          <textarea name="notes" rows={3} placeholder="Notes" className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500" />
      ...
      <Modal isOpen={editingClient !== null} onClose={() => setEditingClient(null)} title="Modifier le client">
        {editingClient && (
          <form onSubmit={handleUpdate} className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              <input name="firstName" required defaultValue={editingClient.firstName} placeholder="Prenom *" className="rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500" />
              <input name="lastName" required defaultValue={editingClient.lastName} placeholder="Nom *" className="rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500" />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <input name="phone" defaultValue={editingClient.phone ?? ''} placeholder="Telephone" className="rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500" />
              <input name="email" type="email" defaultValue={editingClient.email ?? ''} placeholder="Email" className="rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500" />
            </div>
            <textarea name="notes" rows={3} defaultValue={editingClient.notes ?? ''} placeholder="Notes" className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500" />
            <div className="flex justify-end gap-3 pt-2">
              <button type="button" onClick={() => setEditingClient(null)} className="rounded-md border border-gray-300 px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">
                Annuler
              </button>
